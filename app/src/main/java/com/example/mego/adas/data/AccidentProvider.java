...
public class AccidentProvider extends ContentProvider {
    ...
    public Cursor query(@NonNull Uri uri, @Nullable String[] projection, @Nullable String selection,
        ...
        switch (match) {
            case ACCIDENTS:
                ...
                cursor = database.query(AccidentsEntry.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder);
                break;
    ...
    public String getType(@NonNull Uri uri) {
        ...
        switch (match) {
            case ACCIDENTS:
                return AccidentsEntry.CONTENT_LIST_TYPE;
            case ACCIDENT_ID:
                return AccidentsEntry.CONTENT_ITEM_TYPE;
            default:
    ...
    private Uri insertAccident(Uri uri, ContentValues values) {
        ...
        String title = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_TITLE);
        if (title == null) {
            throw new IllegalArgumentException("Accident requires a title");
        }

        //Check the time is not null
        String time = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_TIME);
        if (time == null) {
            throw new IllegalArgumentException("Accident requires a time");
        }

        //Check if the date is not null
        String date = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_DATE);
        if (date == null) {
            throw new IllegalArgumentException("Accident requires a date");
        }

        //Check if the accident id is not null
        String accidentId = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_ID);
        if (accidentId == null) {
    ...
    public int delete(@NonNull Uri uri, @Nullable String selection, @Nullable String[] selectionArgs) {
        ...
        switch (match) {
            case ACCIDENTS:
                ...
                rowsDeleted = database.delete(AccidentsEntry.TABLE_NAME, selection, selectionArgs);
                break;
            case ACCIDENT_ID:
    ...
    private int updateAccident(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(AccidentsEntry.COLUMN_ACCIDENT_TITLE)) {
            String title = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_TITLE);
            if (title == null) {
                throw new IllegalArgumentException("Accident requires a title");
            }
        ...
        //Check the time is not null
        if (values.containsKey(AccidentsEntry.COLUMN_ACCIDENT_TIME)) {
            String time = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_TIME);
            if (time == null) {
                throw new IllegalArgumentException("Accident requires a time");
            }
        ...
        if (values.containsKey(AccidentsEntry.COLUMN_ACCIDENT_DATE)) {
            String date = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_DATE);
            if (date == null) {
                throw new IllegalArgumentException("Accident requires a date");
            }
        ...
        //Check if the accident id is not null
        if (values.containsKey(AccidentsEntry.COLUMN_ACCIDENT_ID)) {
            String accidentId = values.getAsString(AccidentsEntry.COLUMN_ACCIDENT_ID);
            if (accidentId == null) {
                throw new IllegalArgumentException("Accident requires an ID");
            }
        ...
        int rowsUpdated = database.update(AccidentsEntry.TABLE_NAME, values, selection, selectionArgs);
